"""The published artifacts must contain only the ``oubliette_trap`` package.

``oubliette-trap`` is a public PyPI package. The repo also holds things that
have no place in it: ``tests/``, the ``Dockerfile``, CI config and any local
``.env`` or key material.

This is an allowlist check: anything outside the expected layout fails.
This module deliberately imports nothing from ``oubliette_trap`` so the publish
...
test run. Set ``TRAP_REQUIRE_DIST=1`` (the publish workflow and the CI build job do) to
...
def _require_or_skip(artifacts: list[Path], kind: str) -> None:
    if artifacts:
        return
    if os.getenv("TRAP_REQUIRE_DIST") == "1":
        pytest.fail(f"TRAP_REQUIRE_DIST=1 but no {kind} found in dist/")
