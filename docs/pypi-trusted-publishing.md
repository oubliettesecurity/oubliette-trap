...
`oubliette-trap` publishes to PyPI from this repo (`oubliettesecurity/oubliette-trap`) via
...
1. Sign in at [pypi.org](https://pypi.org) as a maintainer of **oubliette-trap** and open
   https://pypi.org/manage/project/oubliette-trap/settings/publishing/
2. Under **Add a new publisher** → **GitHub**, set:

   | Field | Value |
   |---|---|
   | Owner | `oubliettesecurity` |
   | Repository name | `oubliette-trap` |
...
1. Bump `version` in `pyproject.toml` and `__version__` in `src/oubliette_trap/__init__.py`
...
3. `Publish to PyPI` checks that the tag equals `v` + the pyproject version,
   ...
   `tests/test_packaging_boundary.py` against the built files (TRAP_REQUIRE_DIST=1, fail closed), uploads with PEP 740
